...
export function SignupForm() {
  ...
  if (status === "success") {
    const timer = setTimeout(() => {
      window.location.href = "https://chat.whatsapp.com/I7lmbjsMwVPKAYjSzeMBHF";
    }, 1800); // delay = better UX
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const payload = {
      ...
      preferred_move_in_date: data.moveInDate,
    };

    const res = await ApiSocket.post("/auth/waitlist-signup", payload);
  ...
  if (status === 'success') {
    return (
      <motion.div
        ...
        <p className="text-gray-600 text-lg max-w-md mx-auto">
          You're being redirected to our WhatsApp group where you'll get faster updates on available housing.
        </p>
        <p className="text-sm text-gray-400 mt-3">
          If you're not redirected, <a href="https://chat.whatsapp.com/I7lmbjsMwVPKAYjSzeMBHF" className="underline">click here</a>.
        </p>
      </motion.div>);
